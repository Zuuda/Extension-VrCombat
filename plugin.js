(function() {
    if (!window.SillyTavern) {
        console.error('SillyTavern not detected');
        return;
    }

    const VRCombatSimulator = {
        createMob: (level, type) => {
            // Your exact mob creation formulas
            const baseStats = {
                hp: 15 + 5 * level,
                atk: 3 + 1.5 * level,
                def: 2 + 1 * level,
        ...
        runCombat: (player, enemies) => {
            // Your full combat logic here (without UI code)
            // ... [identical to previous implementation] ...
            return combatResult;
        }
    };

    SillyTavern.onPluginLoaded(() => {
        if (!SillyTavern.getContext().isToolCallingSupported()) {
            console.warn('Function calling not enabled');
            return;
        }

        SillyTavern.getContext().registerFunctionTool({
            name: "vrCombatSimulator",
            description: "Calculate combat outcomes between player and enemy groups",
            parameters: {
        ...
        console.log('VR Combat Simulator registered');
    });
})();
